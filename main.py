...
ML_ENABLE_AFTER = int(os.getenv("ML_ENABLE_AFTER", os.getenv("ML_MIN_TRADES_TO_ENABLE", 50)))  # completed trades required before ML can gate entries
...
# Candle data (runner scan + ATR trailing)
CANDLE_GRANULARITY = int(os.getenv("CANDLE_GRANULARITY", 60))
CANDLE_POINTS = int(os.getenv("CANDLE_POINTS", 120))
...
MAX_TRADE_DURATION_MINUTES = int(os.getenv("MAX_TRADE_DURATION_MINUTES", 1440))
# Optional profit target: if set >0, bot will take profit when price >= entry*(1+target%)
PROFIT_TARGET_PERCENT = float(os.getenv("PROFIT_TARGET_PERCENT", 0.0))
# If you want to block tiny “scratch” exits, increase TRAILING_START_PERCENT
...
_candle_cache = {}   # sym -> {"ts": epoch, "data": candles}
CANDLE_CACHE_TTL = int(os.getenv("CANDLE_CACHE_TTL", 30))  # seconds

def get_candles(sym: str, granularity: int = CANDLE_GRANULARITY, limit: int = CANDLE_POINTS):
    """Return list of candles: [time, low, high, open, close, volume] (most-recent first per Coinbase)."""
    try:
        now = time.time()
        ...
        params = {"granularity": int(granularity), "limit": int(limit)}
        r = requests.get(url, params=params, timeout=10)
        data = r.json()
        if not isinstance(data, list) or len(data) < max(ATR_PERIOD + 2, 20):
            return None
...
def _atr_percent_from_candles(candles) -> float:
    """ATR% = ATR / last_close * 100."""
    try:
        # candles are most-recent first
        recent = list(reversed(candles[:max(ATR_PERIOD + 1, 20)]))  # oldest->newest
...
def volume_ratio_from_candles(candles) -> float:
    """Last volume / mean volume over previous 20 candles."""
    try:
        recent = list(reversed(candles[:60]))  # oldest->newest
...
def ret_from_candles(candles, bars_back: int) -> float:
    try:
        recent = list(reversed(candles[:max(bars_back+2, 20)]))  # oldest->newest
        closes = [float(c[4]) for c in recent]
        if len(closes) <= bars_back:
            return 0.0
        p0 = closes[-1]
        p1 = closes[-1 - bars_back]
        if p1 <= 0:
            return 0.0
        return float((p0 - p1) / p1)
    except Exception:
        return 0.0
...
def apply_trailing_stop(pos, sym: str, price: float):
    """Hard stop is always active. Trailing activates only after TRAILING_START_PERCENT profit.
    Trail distance is max(TRAIL_DIST_BASE, ATR_MULT * ATR%), capped reasonably.
    """
    # Update peak always (for later trailing), but do not tighten stop until trailing is active.
    ...
    # Only start trailing once we are meaningfully green
    ...
    # ATR-based trail distance (in %), but keep sane bounds
    trail_dist = max(float(TRAIL_DIST_BASE), float(ATR_MULT) * float(atr_pct))
    trail_dist = min(trail_dist, 6.0)  # cap so it doesn't become useless
...
def score_symbol(sym):
    ...
    # Runner boost (breakout + volume expansion)
    # Uses candles because ticker has no volume history.
    candles = get_candles(sym)
    if candles:
        r15 = ret_from_candles(candles, 15)      # 15 minutes if granularity=60
        r60 = ret_from_candles(candles, 60)      # 60 minutes
        vratio = volume_ratio_from_candles(candles)

        # If it's RUNNING and volume is expanding, bump score (you asked for runners).
        if r15 >= 0.03 and vratio >= MIN_VOLUME_RATIO:
            score += 2.0
        elif r15 >= 0.02 and vratio >= MIN_VOLUME_RATIO:
            score += 1.0

        # Avoid late blow-offs: if 60-min return is already huge, don't chase.
        if r60 >= 0.25:
            score -= 2.0
        elif r60 >= 0.15:
            score -= 1.0
    ...
    try:
        ...
        if now - last_scan >= SCAN_INTERVAL:
            ...
            for sym, pos in list(positions.items()):
                ...
                # Optional profit target (lets you "let it ride" with trailing, but still take a big win if hit)
                if float(PROFIT_TARGET_PERCENT) > 0:
                    entry = float(pos["entry"])
                    if entry > 0 and px >= entry * (1.0 + float(PROFIT_TARGET_PERCENT) / 100.0):
                        qty = float(pos["qty"])
                        proceeds = qty * px
                        profit = (px - entry) * qty
                        ...
                        learning["win_count"] += 1 if profit >= 0 else 0
                        learning["loss_count"] += 1 if profit < 0 else 0

                        equity = compute_equity(prices)
                        notify(f"[{INSTANCE_ID}] SELL {sym} px={px:.6f} entry={entry:.6f} profit={profit:.2f} reason=PROFIT_TARGET({PROFIT_TARGET_PERCENT:.2f}%)")
                        record_history("SELL", sym, px, qty, proceeds, profit, cash, equity, pos.get("score"), f"PROFIT_TARGET")
                ...
                # Time-based exit (prevents bags forever). You can set MAX_TRADE_DURATION_MINUTES very high to disable.
                max_age_sec = int(MAX_TRADE_DURATION_MINUTES) * 60
                if max_age_sec > 0 and (int(time.time()) - int(pos.get("opened_ts", int(time.time())))) >= max_age_sec:
                    entry = float(pos["entry"])
                    qty = float(pos["qty"])
                    proceeds = qty * px
                    profit = (px - entry) * qty
                    ...
                    notify(f"[{INSTANCE_ID}] SELL {sym} px={px:.6f} entry={entry:.6f} profit={profit:.2f} reason=MAX_DURATION")
                    record_history("SELL", sym, px, qty, proceeds, profit, cash, equity, pos.get("score"), "MAX_DURATION")
